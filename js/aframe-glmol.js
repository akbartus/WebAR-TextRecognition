    ...
    var GLmol = (function() {
          ...
          function GLmol(data) {
            ...
            GLmol.prototype.create = function(resolve, reject) {
              ...
              //8;
              this.axisDIV = 5;
              // 3 still gives acceptable quality
              this.strandDIV = 6;
            ...
            GLmol.prototype.download = function(resolve, reject) {
              ...
              if (query.substr(0, 4) == 'pdb:') {
                ...
                var uri = "https://files.rcsb.org/view/" + query + ".pdb";
            ...
            GLmol.prototype.drawBondsAsLineSub = function(geo, atom1, atom2, order) {
              ...
              if (order > 1) {
                vs.push(p1.clone().add(delta));
                cs.push(c1);
                vs.push(tmp = mp.clone().add(delta));
                cs.push(c1);
                vs.push(p2.clone().add(delta));
                cs.push(c2);
                vs.push(tmp);
                cs.push(c2);
                vs.push(p1.clone().sub(delta));
                cs.push(c1);
                vs.push(tmp = mp.clone().sub(delta));
                cs.push(c1);
                vs.push(p2.clone().sub(delta));
                cs.push(c2);
                vs.push(tmp);
                cs.push(c2);
              }
              ...
              for (var i in atomlist) {
                ...
                if ((atom.atom == 'O' || atom.atom == 'CA') && !atom.hetflag) {
                  if (atom.atom == 'CA') {
                    ...
                    O.sub(currentCA);
                    O.normalize();
                    // can be omitted for performance
              ...
              for (var i in atomlist) {
                ...
                if ((atom.atom == 'O3\'' || atom.atom == 'OP2') && !atom.hetflag) {
                  if (atom.atom == 'O3\'') { // to connect 3' end. FIXME: better way to do?
                    ...
                    var O = new TV3(atom.x, atom.y, atom.z);
                    O.sub(currentO3);
                    O.normalize().multiplyScalar(nucleicAcidWidth);
                    // TODO: refactor
            ...
            GLmol.prototype.getChain = function(atomlist, chain) {
              ...
              // concat if Array
              console.log("Chain things", "Length", chain.length, "value", chain);
            ...
            GLmol.prototype.colorByChain = function(atomlist, colorSidechains) {
              for (var i in atomlist) {
                ...
                if (colorSidechains || atom.atom == 'CA' || atom.atom == 'O3\'') {
                  var color = new TCo(0);
                  color.setHSL((atom.chain.charCodeAt(0) * 5) % 17 / 17.0, 1, 0.2);
                  atom.color = color.getHex();
                }
              }
            };
            ...
            GLmol.prototype.drawColor = function(colorMode) {
            ...
            GLmol.prototype.drawBase = function(hetatmMode) {
              var all = this.getAllAtoms();
            ...
            GLmol.prototype.drawHETATM = function(hetatmMode) {
              var all = this.getAllAtoms();
            ...
            GLmol.prototype.defineRepresentation = function() {
              ...
              var biomtChains = this.protein.biomtChains || undefined;
            //  this.getChain(all, biomtChains);
